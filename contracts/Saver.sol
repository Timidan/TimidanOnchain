...
contract Timidan is ERC721("TIMIDAN", "TIMI"), Ownable {
    constructor() {
        _mint(msg.sender, 0);
    }
    ...
    function saveData(
        ...
        require(
            _b.length < 24576,
            "SvgStorage: Exceeded 24,576 bytes max contract size"
        );
    ...
    function genMetadata() public view returns (string memory metadata) {
        string memory key = "dan";
        metadata = string(
            abi.encodePacked("data:application/json;base64,", getData(key))
        );
    }

    function genRawImage() public view returns (string memory metadata) {
        string memory key = "image";
        metadata = string(
            abi.encodePacked("data:image/svg+xml;base64,", getData(key))
        );
    }
